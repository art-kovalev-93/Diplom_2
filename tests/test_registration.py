...
class TestRegistrationApi:
    ...
    @allure.title('Успешная регистрация пользователя, проверка кода ответа 200, проверка тела ответа accessToken и refreshToken.')
    def test_registration_success_200(self, new_user):
        assert new_user.status_code == 200 and 'accessToken' in new_user.json() and 'refreshToken' in new_user.json()
    ...
    @allure.title('Регистрация пользователя с повторяющейся почтой, проверка кода ответа 403, проверка тела ответа.')
    def test_registration_exists_user_403(self, new_user):
        user = UserApi()
        response = user.registration(body=USER_DATA)
        assert response.status_code == 403 and response.json() == USER_EXISTS_ERR

    @allure.title('Регистрация пользователя с не полными данными, проверка кода ответа 403, проверка тела ответа ошибки.')
    ...
    def test_registration_with_empty_param_403(self, body):
        ...
        assert response.status_code == 403 and response.json() == EMPTY_PARAM_ERR
