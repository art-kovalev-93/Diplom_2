...
class TestUserDataApi:
    ...
    @allure.title('Получение данных пользователя, проверка кода ответа 200.')
    ...
    @allure.title('Получение данных пользователя, проверка тела ответа.')
    def test_get_user_data_success_body(self, new_user):
        user = UserApi()
        response = user.get_user_data(access_token=new_user.json().get('accessToken'))
        assert response.json() == SUCCESS_LOGIN

    @allure.title('Изменение данных пользователя, проверка кода ответа 200.')
    ...
    @allure.title('Изменение данных пользователя, проверка тела ответа.')
    @pytest.mark.parametrize('body', PATCH_DATA)
    def test_patch_user_data_success_body(self, new_user, body):
        user = UserApi()
        value = next(iter(body.values()))
        response = user.patch_user_data(access_token=new_user.json().get('accessToken'), body=body)
        assert value in response.json()['user']['email'] or value in response.json()['user']['name']

    @allure.title('Изменение данных пользователя без авторизации, проверка кода ответа 401.')
    @pytest.mark.parametrize('body', PATCH_DATA)
    ...
    @allure.title('Изменение данных пользователя без авторизации, проверка тела ответа.')
    @pytest.mark.parametrize('body', PATCH_DATA)
    def test_patch_user_data_no_auth_body(self, new_user, body):
        user = UserApi()
        response = user.patch_user_data(access_token='', body=body)
